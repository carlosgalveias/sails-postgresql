...
const convertSchema = function(schema) {
  let attr = schema.attributes;
  let newSchema = {}
  for (let key of Object.keys(attr)) {
    if (attr[key].collection) {
      continue;
    }
    if (key === 'id') {
      newSchema.id = {
...
module.exports = (function() {
  ...
  var adapter = {
    ...
    registerConnection: function(connection, collections, cb) {
      ...
      // Describing takes a lot of time, so lets trust the schema is ok for the database
      // and create the internal schema from that

      for (let table of Object.keys(collections)) {
        var connectionObject = connections[connection.identity];
        var collection = connectionObject.collections[table];
        collection.schema = convertSchema(schema[table])
      }
  ...
  function spawnConnection(connectionName, logic, cb) {
    ...
    const pool = initPool(connectionConfig)
